...
## 🎨 Bright Light Mode Design System & Color Tokens

### Visual Identity Keywords for Stitch
`Crisp Light Mode`, `Airy Minimalist Bento Grid`, `Pure White Cards`, `Sapphire Blue & Mint Accents`, `High Contrast Dark Slate Typography`, `Subtle Micro-Shadows`, `Pastel Status Pills`.

### Color Palette Tokens
- **Canvas Base**: Soft Pearl Off-White (`#F8FAFC` / `#F1F5F9`)
- **Card Surfaces**: Pure Snow White (`#FFFFFF` with `border: 1px solid #E2E8F0` and `box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05)`)
- **Primary Brand Accent**: Vibrant Electric Sapphire Blue (`#2563EB` / `#1D4ED8`) — for primary buttons, active tabs, and main navigation icons.
- **Financial Growth Accent**: Mint Emerald (`#10B981` / `#059669`) — for positive revenue trends, profit margins, and paid badges (`LUNAS`).
- **Alert / Overdue**: Bright Coral Rose (`#EF4444` / `#DC2626`) — for overdue invoices and alert badges.
- **Typography**: Dark Slate Charcoal (`#0F172A` headings, `#334155` body text) on *Plus Jakarta Sans* / *Inter*.
- **Pill Badges**: Soft Pastels (Mint `#D1FAE5`, Indigo `#E0E7FF`, Amber `#FEF3C7`, Rose `#FEE2E2`).

---

## 📋 Copy-Paste Prompts for Google Stitch (Bright Light Theme Default)
...
Top Row (4 Bento Metric Cards with high-contrast dark typography #0F172A):
- Card 1 (Omzet Bruto): Big numeric 'Rp 245.850.000' in dark slate text, with a vibrant mint-green trend line underneath and '+14.2%' green pastel badge.
- Card 2 (Laba Bersih): 'Rp 82.400.000' in bold text with 'Margin 33.5%' soft indigo badge (#E0E7FF background, #4F46E5 text).
- Card 3 (Piutang Usaha): 'Rp 18.500.000' with soft red alert badge '4 Faktur Overdue'.
- Card 4 (Total Kas): 'Rp 134.200.000' with connected bank pills (BCA, Mandiri, Kas Utama).

Middle Section (Bento Grid):
- Main Chart: Bright, ultra-clean dual-line area chart (Vibrant Mint Green line for Income, Soft Slate Blue line for Expense) on a white grid with cursor tooltip 'Estimasi PPh Final 0.5%: Rp 1.229.250'.
- Quick Actions Panel: Primary Action button '+ Buat Faktur' in vibrant electric sapphire blue (#2563EB), secondary white button '+ Catat Expense' with subtle border, and WhatsApp share shortcut.

Bottom Section: Clean transaction data table on white background with crisp alternating rows, status badges (Soft Green pill for LUNAS, Soft Red pill for JATUH TEMPO), clear IDR currency formatting, and one-click 'Kirim WA' action button.
```
...
Below QRIS: Bank Virtual Account card displaying Bank BCA VA '880123456789' with a 1-click 'Salin Nomor' button.
Bottom: Pulsing live status indicator '• Menunggu Pembayaran dari Pelanggan (Realtime Sync)'.
```
