...
import { useCreateIncome, type IncomeLine } from '@/lib/data/transactions';
...
export default function NewIncomePage() {
  ...
  return (
    <>
      <div>
        <h2 className="text-headline-lg font-headline-lg text-slate-heading">{t.income.formTitle}</h2>
        <p className="text-body-md font-body-md text-slate-body">{t.income.subtitle}</p>
      </div>

      {/* ------------------------------------------------------------- */}
      {/* DESKTOP VERSION (screen >= md) - Retain existing single-page  */}
      {/* ------------------------------------------------------------- */}
      <form onSubmit={onSubmitDesktop} className="hidden md:grid md:grid-cols-1 gap-6 xl:grid-cols-12">
        ...
        <div className="flex gap-3 xl:col-span-7">
          <IonButton type="submit" disabled={createIncome.isPending}>
            {createIncome.isPending && <IonSpinner name="crescent" slot="start" />}
            {t.common.save}
          </IonButton>
